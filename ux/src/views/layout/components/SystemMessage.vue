...
export default {
  ...
  methods: {
    ...
    checkCRMDetail(type, id, dataIndex, data) {
      if (type === 'schedule') {
        ...
        kmDocDetailRemindAPI({ documentId: id })
          .then(res => {
            const data = res.data || {}
            if (data.type == 'share') {
              window.open(`${WKConfig.getLocationOrigin()}/share/#/${data.data}`)
      ...
      } else if (data.type == 555) { // 密码规则变更
        this.needReset = true
        this.resetRule = {
          isExpire: false, // 密码过期
          ruleChange: true // 密码规则变更
        }
      } else {
        this.relationID = id
