<template>
  <wk-head-section
    ...
    <examine-create-view
      v-if="oaCreateShow"
      ref="oaAllCreate"
      :category-id="crmCreateAction.data.categoryId"
      :type="crmCreateAction.data.type"
      :action="crmCreateAction"
      :props="createProps"
...
export default {
  ...
  computed: {
    ...
    // 字段授权审批  展示审批按钮
    isFieldAuthExamine() {
      return this.isCRMExamine || this.isOAExamine
    },
  ...
  methods: {
    ...
    async examineClick() {
      ...
      if (this.isCRMExamine) {
        ...
        const res = await detailRequest(this.id)
        const resData = res.data
        if (resData) {
          const crmLabel = {
            1: 6,
          ...
          const getFields = () => {
            return new Promise((resolve, reject) => {
              this.flowGetAuthFields({
                ...
                resolve(objDeepCopy(this.authFields))
              }).catch(() => {
                reject()
              })
            })
        ...
        const res = await this.flowGetOADetailRequestFun()(this.id)
        const resData = res.data
        if (resData) {
          const getFields = () => {
            return new Promise((resolve, reject) => {
